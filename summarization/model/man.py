...
from .multihead_attention2 import MultiheadAttention2 as MultiheadAttention
...
@register_model('man')
class MANModel(FairseqEncoderDecoderModel):
    """
    Args:
    ...
    def add_args(parser):
        ...
        parser.add_argument('--encoder-mix-layers', type=int, metavar='N',
                            help='num mixed encoder layers')
        ...
        parser.add_argument('--decoder-mix-layers', type=int, metavar='N',
                            help='num mixed decoder layers')
    ...
    def build_model(cls, args, task):
        ...
        model = MANModel(encoder, decoder)
    ...
    def forward(self, src_tokens=None, src_lengths=None, prev_output_tokens=None, **kwargs):
        ...
        """
        # print('| src_tokens, prev_output_tokens', src_tokens.size(), prev_output_tokens.size())
...
class MANEncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if self.mix:
            self.pre_self_attn = MultiheadAttention(
                ...
                self_attention=True,
                re_weight_m=1, max_len=max_source_positions,
                activation=self.activation_fn, activation_dropout=self.activation_dropout
            )
        else:
            self.pre_fc1 = nn.Linear(self.embedding_dim, ffn_embedding_dim)
            self.pre_fc2 = nn.Linear(ffn_embedding_dim, self.embedding_dim)
        # layer norm associated with the pre-self attention layer
        self.pre_layer_norm = LayerNorm(self.embedding_dim, export=export)
        ...
        self.self_attn = MultiheadAttention(
            ...
            add_zero_attn=add_zero_attn,
            self_attention=True,
            re_weight_m=0, max_len=max_source_positions,
            activation=self.activation_fn, activation_dropout=self.activation_dropout
        )
    ...
    def forward(
        ...
        if not self.mix:
            residual = x
            x = self.activation_fn(self.pre_fc1(x))
...
class MANDecoderLayer(nn.Module):
    def __init__(
            ...
            export: bool = False,
            mix: bool = False,
            max_target_positions: int = DEFAULT_MAX_TARGET_POSITIONS,
    ):
        super().__init__()
        ...
        if self.mix:
            self.pre_self_attn = MultiheadAttention(
                ...
                add_zero_attn=add_zero_attn,
                self_attention=True,
                re_weight_m=1, max_len=max_target_positions,
                activation=self.activation_fn, activation_dropout=self.activation_dropout
            )
        else:
            self.pre_fc1 = nn.Linear(self.embedding_dim, ffn_embedding_dim)
        ...
        self.self_attn = MultiheadAttention(
            ...
            add_zero_attn=add_zero_attn,
            self_attention=True,
            re_weight_m=0, max_len=max_target_positions,
            activation=self.activation_fn, activation_dropout=self.activation_dropout
        )
    ...
    def __init__(self, args, dictionary, embed_tokens):
        ...
        self.layers.extend([
            MANEncoderLayer(
                ...
                args.activation_fn,
                mix=True if i < args.encoder_mix_layers else False,
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.layers.extend([
            MANDecoderLayer(
                ...
                args.activation_dropout,
                args.activation_fn,
                mix=True if i < args.decoder_mix_layers else False,
...
@register_model_architecture('man', 'man_base')
def man_base(args):
...
@register_model_architecture('man', 'man_middle')
def man_middle(args):
...
@register_model_architecture('man', 'man_big')
def man_big(args):
    args.encoder_layers = getattr(args, 'encoder_layers', 12)
    args.encoder_mix_layers = getattr(args, 'encoder_mix_layers', 12)
    args.decoder_layers = getattr(args, 'decoder_layers', 12)
    args.decoder_mix_layers = getattr(args, 'decoder_mix_layers', 12)
    man_middle(args)
